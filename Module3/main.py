...
api_key = 'AIzaSyCJQTkGXzuWHTHYoHwu_ous1CKZf06vzDU'
youtube = build('youtube', 'v3', developerKey=api_key)

# searching for videos based on a specific topic/category
search_response = youtube.search().list(q='music in English', part='id', maxResults=50).execute()
...
for item in search_response['items']:
    ...
    if video_id:
        video_response = youtube.videos().list(id=video_id, part='snippet,statistics,contentDetails').execute()
...
fields = ['title', 'description', 'viewCount', 'likeCount', 'commentCount', 'publishedAt', 'comments', 'duration']
